...
此资源转换脚本主要用于将`Clash`等APP的订阅规则转换成`Quantumult X`的规则。

## 一、使用方式
1. 在`Quantumult X`的配置文件中加入资源解析器
...
2. 在订阅连接后面添加解析参数，格式为: `订阅URL`?`参数列表`,`opt-parser=true...`，例如: https://cdn.jsdelivr.net/gh/RS0485/V2rayDomains2Clash@generated/category-ads-all.yaml?src=clash&dst=quan&type=rule&subtype=domain&policy=REJECT, tag=category-ads-all, opt-parser=true, update-interval=259200
...
#### 2.3.4 ipasn-raw从`bgp.he.net`网页解析生成ASN规则
...
## 四、分流规则订阅资源
- [Clash 分流规则](https://github.com/RS0485/V2rayDomains2Clash/tree/generated)
